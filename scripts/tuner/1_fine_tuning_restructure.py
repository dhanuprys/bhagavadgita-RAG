...
class VerseTranslationEntity:
    ...
    def to_dict(self):
        return {"id": self.id, "content": self.content, "verse_id": self.verse_id}
...
class VerseEntity:
    ...
    translations: List[VerseTranslationEntity] = field(
        ...
        for data in self.db:
            ...
            new_data.append(
                VerseTranslationEntity(
                    id=index, content=data["description"], verse_id=data["verse_id"]
                )
            )
            index += 1
...
class JsonVerseRepository(JsonLoader, VerseRepository):
    ...
    def __init__(
        self, filename, verse_translation_repository: VerseTranslationRepository
    ):
        super().__init__()
        self.load(filename)
...
verse_translation_repository = JsonVerseTranslationRepository(
    "data/raw/translation.json"
)
verse_repository = JsonVerseRepository(
    "data/raw/verse.json", verse_translation_repository
)
...
with open("data/1-fine-restructured/chapters.json", "w") as file:
    file.write(
        json_dumps_hindi(
            [chapter.to_dict() for chapter in chapter_repository.get_all()]
        )
    )
...
with open("data/1-fine-restructured/translations.json", "w") as file:
    file.write(
        json_dumps_hindi(
            [
                translation.to_dict()
                for translation in verse_translation_repository.get_all()
            ]
        )
    )
